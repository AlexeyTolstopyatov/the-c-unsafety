...
int main() {
    // Самый простой способ вызыать неопределенное поведение
    int ub;
    printf("%d", ub);
    
    // целочисленное значение ub обьявленно, но не инициализированно.
    // перед работой с примитивами похоже, как и в С++
    // необходимо их инициализировать
    
    ub = 8192;
    // тогда в результате вывода значения будет
    // 8192 а не любое значение из памяти
    
    printf("%d", ub);
    
    // такая ситцация тоже является доказательством
    // отсуствтвием типобезопасной среды.
    // Внутри CLR среды неинициализированные значения
    // возвращают специальное значение NULL, как знак пустоты
    // или отсутствия значения. 
    // а численные значения по умолчанию имеют значение 0.
    //
    // int a; // будет инициализирован со значением 0
    // float f; // 0.0f
    
    // мне всетаки стоит отметить, что строка с обьявлением ub 
    // может и ничего не выводить. 
    // это неопределённое поведение, так что вам повезёт, 
    // если вопрос будет только в выводе и не выводе, 
    // ведь соответствующий стандартам компилятор 
    // теоретически может сгенерировать код, стирающий жёсткий диск например...
    return 0;
}
